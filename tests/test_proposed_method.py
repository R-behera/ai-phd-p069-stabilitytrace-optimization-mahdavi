...
class ProposedMethodTest(unittest.TestCase):
    def test_unique_method_runs(self):
        result = run_method()
        self.assertEqual(result["algorithm_id"], 'P069-Mahdavi-MlOptimization')
        self.assertEqual(result["algorithm_name"], 'MahdaviEvidenceMlOptimizationRepairLoop')
        self.assertEqual(result["project_family"], 'optimization')
    ...
    def test_value_add_report_includes_unique_method(self):
        report = run_demo()
        self.assertEqual(report["proposed_method"]["algorithm_id"], 'P069-Mahdavi-MlOptimization')
