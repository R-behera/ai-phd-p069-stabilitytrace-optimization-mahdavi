...
This repo has its own runnable prototype method: **MahdaviEvidenceMlOptimizationRepairLoop**.
...
The method ranks the seed cases using a professor-aligned `optimization` decision rule. See `docs/unique_algorithm.md` for the mechanism and honest limitations.
...
The code runs a `optimization`-specific audit over `data/value_add_examples.csv` using professor metadata from `data/advisor_profile.json`. The sample data is intentionally small and honest; replace it with real public/lab-relevant data before making research claims.
...
- **Program:** CSE PhD
- **CSV research area:** ML optimization, federated learning, learning theory
